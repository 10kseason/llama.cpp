"""Exact P6 wide GEMV versus original, P6 AVX2/VNNI and scalar independent oracle."""
import unittest
import numpy as np
...
class WideKernelTests(unittest.TestCase):
    cases = 0
    output_floats = 0
    ...
    def setUpClass(cls):
        ...
        if not cls.dll.q4kp_wide_supported():
            raise unittest.SkipTest("wide ISA unavailable")
    ...
    def compare(self, n, nc, packed, q8):
        ...
        outs = [np.full(nc + 2, np.nan, dtype=np.float32) for _ in range(5)]
